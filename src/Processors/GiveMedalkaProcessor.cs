...
namespace MedalsBot.Processors
{
    public abstract class GiveMedalkaProcessor
    {
        public static async Task ProcessGiveMedalkaAsync(Message message, TelegramBotClient? bot, Database? db)
        {
            ...
            if (!int.TryParse(parts[1], out int medalTypeInt) || medalTypeInt is < 1 or > 5)
            {
                if (bot != null)
            ...
            string explanation = parts.Length >= 3 && !string.IsNullOrWhiteSpace(parts[2])
                ? parts[2]
                : "<i>null</i>"; // fallback when explanation is missing or empty
            string originalMessageText = message.ReplyToMessage.Text ?? "<i>нет сообщения</i>";

            // recipient info from the replied-to message
            long recipientUserId = message.ReplyToMessage.From.Id;
            string? recipientUsername = message.ReplyToMessage.From.Username;
